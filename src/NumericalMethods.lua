...
function NumericalMethods.solvers.regulaFalsi (f : (number) -> number, t : number, a : number, b : number, tol : number) : number
...
function NumericalMethods.solvers.newtonsMethod (f : (number) -> number, fPrime : (number) -> number, t : number, x : number, tol : number) : number
    tol = tol or 10^(-13)
    x = x or 0
    local value = f(x) - t
...
function NumericalMethods.integration.adaptiveQuadrature (f : (number) -> number, a : number, b : number, tolerance : number, guess : number) : number
    ...
    if math.abs(guess - splitGuess) < tolerance then
        ...
        return NumericalMethods.integration.adaptiveQuadrature(f, a, midPoint, tolerance, left) + NumericalMethods.integration.adaptiveQuadrature(f, midPoint, b, tolerance, right)
    end
end

NumericalMethods.integration.integrate = NumericalMethods.integration.adaptiveQuadrature

NumericalMethods.integration.fivePointLaguerreGrid = {0.2635603197181409, 1.4134030591065208, 3.5964257710407237, 7.085810005858842, 12.640800844275782}
NumericalMethods.integration.fivePointLaguerreWeights = {0.5217556105828091, 0.3986668110831633, 0.07594244968170724,0.003611758679922023, 0.000023369972385776235}
...
function NumericalMethods.integration.adaptiveLaguerre (f : (number) -> number, a : number, negative : boolean, tolerance : number, guess : number, stride : number, strideTolerance : number) : number
    tolerance = tolerance or 10^(-13)
    stride = stride or 1
    ...
    if stride > strideTolerance then
        error("Adaptive Laguerre Quadrature failed to converge! Either try a higher strideTolerance or the integral may not converge.")
    end
    if negative then
        ...
        if math.abs(guess - splitGuess) < tolerance then
            ...
            return left + NumericalMethods.integration.adaptiveLaguerre(f, a - stride, negative, tolerance, right, 2 * stride, strideTolerance)
        end
    else
...
function NumericalMethods.integration.trapezoid (f : (number) -> number, a : number, b : number, tolerance : number, guess : number, table : { [number] : number }) : number
    local width = b - a
    if table then
...
function NumericalMethods.integration.adaptiveTrapezoid (f : (number) -> number, a : number, b : number, tolerance : number, guess : number, table : { [number] : number }) : number
    ...
    if math.abs(guess - splitGuess) < tolerance or width < tolerance then
        ...
        return NumericalMethods.integration.adaptiveTrapezoid(f, a, midPoint, tolerance, left, table) + NumericalMethods.integration.adaptiveTrapezoid(f, midPoint, b, tolerance, right, table)
    end
end
