...
function ODE.SpectralMethods.chebyshevSecondOrderBoundaryValueProblem(
	coeffList,
	f,
	boundary,
	boundaryValues,
	...
	for i, v in ipairs(coeffList) do
		if type(v) == "function" then
			coeffList[i] = Tools.list.map(function(t)
				return ((x - y) / 2) ^ (i - #coeffList) * t
			end, Tools.list.map(v, Tools.list.map(_linearRescalingFunction(y, x), chebyshevGrid)))
...
function ODE.SpectralMethods.chebyshevSecondOrderInitialValueProblem(
	coeffList,
	f,
	boundary,
	boundaryValues,
