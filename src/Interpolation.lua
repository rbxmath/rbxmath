...
local _barycentricInterpolationSolve = function (fList, t, tol, chebyshevGridPoints)
    local n = #fList
    local chebyshevGrid = chebyshevGridPoints or _chebyshevGrid(n)
    local tolerance = tol or (10^-14)
    ...
    for i = 1, n - 1, 1 do
        if t <= fList[i + 1] - tolerance and t >= fList[i] + tolerance then
    ...
    while #intervalList ~= 0 do
        ...
        for i, v in ipairs(intervalList) do
            ...
            if math.abs(f2) < tolerance then
                ...
                local gap
                while math.abs(f2) >= tolerance and gap ~= (x1 - x0) do
                    gap = x1 - x0
                ...
                if math.abs(f2) < tolerance or gap == (x1 - x0) then
                    rootList[#rootList+1] = x2
                end
            end
        end
...
Interpolation.Chebyshev.solve = function (f, t, n, tol, chebyshevGridPoints)
    local chebyshevGrid = chebyshevGridPoints or _chebyshevGrid(n - 1)
    local tolerance = tol or (10^-14)
    local fList = {}
    for i = 1, n, 1 do
...
Interpolation.Chebyshev.solveFromData = function (fList, t, tol, chebyshevGridPoints)
    local n = #fList
    local chebyshevGrid = chebyshevGridPoints or _chebyshevGrid(n - 1)
    local tolerance = tol or (10^-14)
