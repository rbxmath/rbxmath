...
local _chebyshevGrid = function(n: number, a: number, b:number): Vector
   local result = table.create(n + 1)
   a = a or -1
   b = b or max(a, 1)
   local widthOverTwo = (b - a) / 2

   for i = 0, n, 1 do
      result[n + 1 - i] = widthOverTwo * (math.cos(i * math.pi / n) + 1) + a
   end
...
local function _chebyshevSpectralDifferentiationMatrix(n: number, chebyshevGridPoints: Vector): Tensor
   local chebyshevGrid = chebyshevGridPoints or _chebyshevGrid(n)
   local d = { {}, {} }
...
local function _chebyshevHigherOrderSpectralDifferentiationMatrix(
      n: number,
      p: number,
      chebyshevGridPoints: Vector
							      ): Tensor
...
function ChebyshevInterpolant:new(
   ...
   chebyshevGrid = chebyshevGrid or _chebyshevGrid(n, a, b)
   local fList
   if type(f) == "function" then
      fList = table.create(n + 1)
      for i = 1, n + 1, 1 do
...
function ChebyshevInterpolant:adaptive(f: ScalarMap, a: number, b: number, tol: number): Object
   ...
   while norm >= tol do
      n *= 2
      currentGrid = _chebyshevGrid(n, a, b)
      for i = n + 1, 1, -1 do
	 if i % 2 == 1 then
	    currentGridValues[i] = currentGridValues[(i - 1) / 2 + 1] or f(currentGrid[i])
	 else
	    currentGridValues[i] = f(currentGrid[i])
	 end
      end
...
function ChebyshevInterpolant:evaluate(x: number): number
   ...
   local xDiffList = table.create(n)
   local diff
   for i, v in ipairs(chebyshevGrid) do
      diff = x - v
      if diff == 0 then
   ...
   for i = 2, n - 1, 1 do
      val = xDiffList[i]
      numerator += fList[i] * val
      denominator += val
   end
   val = 0.5 * xDiffList[n]
   numerator += fList[n] * val
   denominator += val
...
function ChebyshevInterpolant:solve(t: number, tol: number, method: string, gapTol: number): number
   ...
   if method == "Monotone" then
      return _barycentricInterpolationSolve(self.gridValues, t, tol, gapTol, self.grid)
   elseif method == "RegulaFalsi" then
	 ...
	 print("Grid values")
	 print(Tools.list.tostring(derivative.gridValues))
	 if math.max(table.unpack(Tools.list.map(math.abs, derivative.gridValues))) >= tol then
	    for j = 1, #boundList - 1, 1 do
	       ...
	       end, 0.5 * (boundList[j] + boundList[j + 1]), boundList[j], boundList[j + 1], tol)
	       print("Moose\nMoose\nMoose")
	       print(Tools.list.tostring(newBoundList))
	    end
...
function ChebyshevInterpolant:derivative(n: number): Object
   n = n or 1
   local derivativeMatrix = _chebyshevHigherOrderSpectralDifferentiationMatrix(self.degree, 1, self.grid)[2]
   local fVector = {}
   for key, value in ipairs(self.gridValues) do
      fVector[key] = { value }
   end
   local fColumn = Matrix:new(fVector)
   for i = 1, n, 1 do
...
function ChebyshevInterpolant:max(method: string): number
   ...
   local extrema = derivative:solve(0.5 * (self.leftBound + self.rightBound), nil, method)
...
function ChebyshevInterpolant:inverse(method: string): Object
   ...
   print(max, min)
   print(self.solveMethod)
   local fList = {}
   for key, value in ipairs(self.grid) do
      local save = self:solve(value, nil, method)
      print(Tools.list.tostring(save))
...
Interpolation.Chebyshev.derivativeMatrix = _chebyshevSpectralDifferentiationMatrix

Interpolation.Chebyshev.derivativeMatrices = _chebyshevHigherOrderSpectralDifferentiationMatrix
